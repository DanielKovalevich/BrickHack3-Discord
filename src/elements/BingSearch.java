...
public class BingSearch extends ListenerAdapter {
 ...
 //       request.setEntity(params);
        ...
        @Override public void onMessageReceived(MessageReceivedEvent event) {
        	...
        	if(event.getMessage().toString().startsWith("!search")) {
        		
        		String message = event.getMessage().toString().substring(7);
        		try {
...
//    }
	...
	//public void doCommand(MessageReceivedEvent event) {
		//
		//	You have to get the search from the event here. Not sure what the string for this
		//	command is.
		//
		//try {
		//	search();
